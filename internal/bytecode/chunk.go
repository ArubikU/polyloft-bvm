...
func (c *Chunk) Disassemble(name string) string {
	...
	for offset := 0; offset < len(c.Code); {
		...
		switch op {
		case OpConstant, OpDefineGlobal, OpGetGlobal, OpSetGlobal, OpGetProperty, OpSetProperty:
			idx := readUint16(c.Code[offset+1:])
			fmt.Fprintf(&out, "%d (%v)", idx, c.Constants[idx])
			offset += 3
		case OpGetLocal, OpSetLocal, OpCall, OpCallSuper, OpRange, OpIterInit, OpTuple, OpUnpack, OpGetField, OpSetField, OpDefineGlobalSlot, OpGetGlobalSlot, OpSetGlobalSlot:
